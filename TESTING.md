...
**Known limitation:** `createTestDb()` re-applies every migration file from scratch per test file (not per test) — fine at the current migration/test-file count, would need reconsideration if either grows large enough to slow the suite down noticeably. Running 5 PGlite instances concurrently (vitest parallelizes across test files by default) is CPU-heavy enough that the default 10s `hookTimeout` isn't always enough under contention — raised to 30s in `vitest.config.ts` rather than forcing serial file execution, which would slow every file down, not just the integration ones. Full suite currently takes roughly 5-25s wall-clock depending on machine load, almost entirely PGlite startup/migration cost.
