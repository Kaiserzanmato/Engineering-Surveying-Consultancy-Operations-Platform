...
| Broken access control / horizontal leakage | All scoped resources (leads, projects, and eventually project-scoped sub-resources) | Single server-side authorization function (`src/lib/auth/authorize.ts`) implementing the ALLOW rule, called in every protected layout/page/server action — **now the sole enforcement point, not defense-in-depth on top of middleware** (see below) | **Implemented**, exercised twice now against two differently-shaped scopes: `administrative_staff` can only view/edit/convert leads assigned to them (`src/lib/crm/leads.ts`'s `leadInScope`, single-owner `assignedTo` field); and, since the Projects slice, project access is scoped via `ProjectMember` (many-to-many), `src/lib/projects.ts`'s `projectInScope`/`projectMembersInScope` — out-of-scope leads/projects return 404. A 2026-08-11 review of the Projects scope further separated *project editing* from *project-membership administration* into two permissions (`projects:manage` vs `projects:manage_members`) after finding the original single-permission design let a scoped member alter other members' access, not just project data — see `docs/security/RBAC_MATRIX.md`'s "Project membership administration" section |
...
3. **Resolved 2026-08-11**: `resourceInScope` is now exercised against a second, differently-shaped case — `ProjectMember` (many-to-many) vs. Leads' single-owner `assignedTo`. The composition assumption flagged here previously (that lead-scoping working doesn't guarantee project-scoping would too) was tested directly: a unit test calling `projectMembersInScope` in isolation caught a real design flaw (a membership-based fallback that was only safe by accident of caller order, not by the function's own logic) before it shipped — see `docs/security/RBAC_MATRIX.md`'s "Project membership administration" section for the full account. Residual gap: still only two shapes tested (single-owner, many-to-many); a third differently-shaped scope (e.g. a future hierarchical or time-bounded scope) should get the same direct-unit-test scrutiny, not assumed safe by analogy.
...
- ~~The Projects slice ships and exercises `resourceInScope` for a second, differently-shaped case (resolve gap 3).~~ Done 2026-08-11.
- A third differently-shaped resource scope is added (re-open gap 3's residual note).
