...
## Test coverage (updated 2026-08-11 — see `TESTING.md` for the full strategy)

| Area | Unit coverage | Real-DB integration coverage | Page/route authorization coverage |
|---|---|---|---|
| Identity/RBAC (Users & Roles) | `src/lib/auth/authorize.test.ts` (`evaluateAllow` ALLOW-rule logic), `src/db/schema.test.ts` (role catalog matches PRD §12) | `src/app/(app)/admin/users/actions.integration.test.ts` | `page-authorization.integration.test.ts` (Users/Admin section) |
| Leads | `src/lib/crm/leads.test.ts` | `src/app/(app)/crm/leads/actions.integration.test.ts` | `page-authorization.integration.test.ts` (Leads section) |
| Clients | — (`parseAddressFields`/`address.test.ts` covers form parsing, not RBAC) | `src/app/(app)/crm/clients/actions.integration.test.ts` | `page-authorization.integration.test.ts` (Clients section) |
| Projects | `src/lib/projects.test.ts` | `src/app/(app)/projects/actions.integration.test.ts` | `page-authorization.integration.test.ts` (Projects section) |
| Field Data, Master Repository, Technical Files, Billing, Audit, System Settings | none | none | none — **feature doesn't exist yet**, not a test-coverage gap |

All integration/page-authorization tests run against a real (ephemeral, in-memory, PGlite-backed) Postgres instance seeded from this file's own source of truth (`scripts/seed.ts`), exercising every role listed below — not a hand-picked subset.
...
- Current answer to **"who can add/remove `ProjectMember` records?"**: only `system_administrator` and `owner_gm`, enforced by `projects:manage_members` (`scripts/seed.ts` grants; verified against the dev database 2026-08-11 — exactly those two roles hold the key, `administrative_staff` does not). Also covered by an automated test: `src/app/(app)/projects/actions.integration.test.ts`'s "negative/security scenarios" group asserts this against a real (ephemeral, in-memory) Postgres instance, not just a one-off manual query.
...
- PRD §12's CRM rows have several "Assigned"/"Limited"/"Full/Assigned" entries that aren't fully mechanical to scope yet (no client-assignment concept exists). Judgment calls made — see the comment block above `grants` in `scripts/seed.ts` for the full reasoning per role; summary: Finance's "Limited" read as read-only (consistent with the Users/Roles reading), Field/CAD/Reviewer's "Assigned" on Clients read as "No" for now (no mechanism to scope it correctly yet — granting unscoped access would be broader than the PRD intends), Admin Staff's "Full/Assigned" on Clients granted as full manage (no client-assignment table to scope against). **Confirmed by test, 2026-08-11** (`src/app/(app)/crm/clients/actions.integration.test.ts`): Administrative Staff can currently read and edit ANY client, including ones created by a different Administrative Staff member — this is the actual enforced behavior, not a theoretical reading of the grants table. Documented as current state, not resolved — whether Point View wants a `ClientMember`/assignment model (the way Projects now has `ProjectMember`) remains an open business-review question, deliberately not invented during this testing pass.
