...
3. **Projects — DONE for its own scope** (2026-08-10): project record (project number, client, service type, location, status, current stage, blocker, dates, billing-status placeholder text field) and `ProjectMember` team roster (add/remove, scoped visibility). Permissions and membership scoping documented in `docs/security/RBAC_MATRIX.md`. Explicitly NOT built: any workflow-stage/checklist/dependency mechanics behind `currentStage` (that's item 4, Workflow engine — `currentStage`/`blocker` are plain manager-set fields for now, not workflow-driven), any real Billing entity behind `billingStatus` (that's item 9 — currently just a free-text field), project deletion/archival, and a searchable/paginated member picker (plain `<select>` of active users, fine at current scale).
