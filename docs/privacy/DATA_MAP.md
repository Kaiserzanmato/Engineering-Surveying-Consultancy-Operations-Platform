...
| Client & contact data | `clients` + `contacts` tables (implemented) — name, type, structured billing address (line1/line2/sub-locality/city/state-province/postal/country — see `src/lib/address.ts`), status; contacts: name, email, phone, title | Personal (client-side individuals) | Service delivery, relationship management, billing | Contract/legitimate interest | `clients:read`/`clients:manage` per `docs/security/RBAC_MATRIX.md` — SysAdmin/Owner/GM/Admin Staff/Sales full read+manage, Finance read-only, Field/CAD/Reviewer no access yet (see RBAC_MATRIX known limitations) | Postgres (`sin1`) | `sin1`/Singapore | TBD | Same as above — no deletion code path yet |
