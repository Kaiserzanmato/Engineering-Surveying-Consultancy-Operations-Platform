...
## Entry 16 — 2026-08-11 — Claude Code (Sonnet 5)

**Role:** Builder/Security Tester — closing the database-level negative-authorization test gap Entry 15 flagged as "not done this pass," per explicit user direction to close it before anything else.

**Objective:** This repo's test suite was 100% pure-unit prior to this entry — no test anywhere touched a real (or test) database. That's fine for pure functions (`projectInScope`, `parseProjectStatus`, etc.), but it meant nothing actually proved the full request-to-response path — `authorize()` → real role/permission joins → real `ProjectMember` rows → real server action — enforces RBAC end to end. Entry 15's own bug (`projectMembersInScope`'s unsafe membership fallback) was caught by a unit test calling the function in isolation; this entry adds the layer that would catch a bug in how the pieces *compose*, not just in one function's logic.

**Approach — no Docker, no network dependency, no risk to the real dev database:** Added `@electric-sql/pglite` (real Postgres compiled to WASM, in-memory, ephemeral — not a mock) as a dev dependency; `drizzle-orm` already ships a `pglite` driver, so no ORM-level shimming was needed. `test/db-test-utils.ts` builds a fresh test database per test file by replaying this repo's actual `drizzle/*.sql` migration files (not a hand-written fixture schema that could drift from what production runs), then seeds it with the exact same role/permission/grant data `scripts/seed.ts` applies to real environments — refactored that file to `export const roles/permissions/grants` (previously local to `main()`) specifically so tests import the real data instead of a hand-maintained copy that could silently diverge. `scripts/seed.ts` itself is unchanged in behavior (verified: re-ran `bun run db:seed` against the real dev DB after the refactor, same "9 roles, 11 permissions, 37 grants" output as before).

**What's mocked vs. real** in `src/app/(app)/projects/actions.integration.test.ts`: only `@clerk/nextjs/server`'s `auth()` (so the test controls who's "signed in") and `@/db`'s `getDb()` (so actions hit the PGlite instance instead of real Neon) are mocked, via `vi.mock` + `vi.hoisted` refs the tests mutate between calls. `next/navigation`'s `redirect()`/`notFound()` are genuinely NOT mocked — read their actual Next.js source before assuming they'd need it, and confirmed both just throw a plain `Error` with a digest string (`"NEXT_REDIRECT;..."` / `"...;404"`) with no request-context/AsyncLocalStorage dependency, so real behavior runs and tests assert on the thrown digest directly. `next/cache`'s `revalidatePath` IS mocked (no-op) — its real implementation does depend on Next's request-scoped async storage and would throw an invariant error outside an actual request.

**A real test bug caught during writing, not an app bug:** first draft of the "administrative_staff cannot add itself to an unrelated project" test failed — but the fault was the test, not `addProjectMember`: a preceding `createProjectAs(otherOwner)` call left the mocked session as `otherOwner` (an unscoped `owner_gm`, who legitimately can add anyone to any project), and the test forgot to switch the mocked session back to the intended `admin` actor before the final assertion. Worth recording because it's exactly the class of mistake this kind of stateful-mock test harness invites — audited every other test in the file for the same "session left over from a helper call" pattern before considering the file done.

**Coverage delivered**, organized by role per the review instruction's Sections 23-24: System Administrator and Owner/GM (unblocked by lack of membership, can manage members); Administrative Staff (creates + auto-membership, scoped read/edit via `updateProject`, denied on unrelated projects, denied `projects:manage_members` entirely including self-add); the four scoped-read-only roles via `it.each` (never permitted to edit or manage members, member or not); Finance (denied edit/manage); Sales (denied everything, including create). Plus four scenario-based security tests: membership removal immediately revokes access (no stale cached authorization — Section 24's explicit ask); a suspended user is denied everything including a former System Administrator; an unknown/never-synced Clerk session id is denied; a denied `updateProject` call doesn't partially write (queried the row after rejection to confirm `status` wasn't changed); project actions never write to `user_roles` (no role-escalation side channel exists structurally, verified rather than assumed).

**Files changed:** `package.json`/`bun.lock` (added `@electric-sql/pglite` as a dev dependency), `scripts/seed.ts` (exported `roles`/`permissions`/`grants`, added the Node-vs-Bun-safe entry-point guard so importing the file for its data doesn't also re-run `main()`), `test/db-test-utils.ts` (new), `src/app/(app)/projects/actions.integration.test.ts` (new, 16 tests), `docs/security/RBAC_MATRIX.md`, `docs/IMPLEMENTATION_PLAN.md` (both updated to point at this instead of describing the gap as open).

**Tests:** `bun run lint`, `bunx next typegen && bunx tsc --noEmit`, `bun run test` — **80/80 passing** (up from 64; +16 new, all in the new integration file), `rm -rf .next && bun run build` clean, confirmed `@electric-sql/pglite` stayed a dev dependency and didn't affect the production route list (still the same 13 routes). No migration/seed change to the real database — this entry only added test infrastructure and a non-behavioral refactor to `scripts/seed.ts`.

**Privacy/security impact:** None new to production — test-only infrastructure, runs against an ephemeral in-memory database that's discarded after the test process exits, never touches real Neon/Clerk. `test/db-test-utils.ts`'s fake Clerk ids (`user_test_<label>_<n>`) and `@example.test` emails are obviously synthetic, not real personal data.

**Unresolved risk / known limitations:**
- This pattern (PGlite-backed integration tests for server actions) exists for Projects only — Leads/Clients/Users remain pure-unit-only. Worth adopting repo-wide, but that's a separate, larger effort than "close the gap the last review flagged," not attempted here.
- Page-level authorization (`src/app/(app)/projects/page.tsx` / `[id]/page.tsx`, React Server Components) is still not integration-tested — only the server actions are. The scoping logic they call (`projectInScope`) is the same function covered by both the unit tests and, indirectly, the integration tests' action-level assertions, so the risk is narrower than it sounds, but rendering the actual page components under test was out of scope for this pass.
- `createTestDb()` re-applies every migration file from scratch per test file (not per test) — fine at current migration count (6 files) and test-file count (2), would need reconsideration if either grows large enough to slow the suite down noticeably.
- Everything from Entry 15 not touched by this entry remains as documented there (transaction/atomicity note, VTA pending, MFA disabled pending Clerk Pro, etc.).
